...
9. 被要求证明论文控制策略的稳定性，创建反步法类，经过论文求证之后是要鸽了。  对应分支`backstepping`，坑了，打算转入观测器的方法。

---

每头文件类的作用：
前向头文件`PMSM_sim.h`：包含整个库的各个类、提供系统参数的定义和坐标变换。  
...
`backstepping.h`：反步法控制器类，但是看了论文存在没有办法证明的假设，该类弃用，以后再说。

---

2023/8/24  - 26  
现在有一个问题就是这个使用反步法的方法没有使用。因为其只是一个控制策略，并不是真正的对于李雅普诺夫函数的递减有用的，这几天看了两种方法。其一为：设计李雅普诺夫函数并针对遍历选取的结果是否满足递减
