...
public:
    ...
    double Iq_ref;
    // 设置状态输入变量的权重,现在只支持所有都设置为一个统一的lambda_u;
    // 如果需要的话,可以将这个值变成一个对角矩阵,来实现更加复杂的功能
...
public: 
    ...
    sda(double A_, double C) : a(A_), c(C)
    {
        ...
        // 得到：Γ 
        // diag(A, A^2, A^3, A^4, .... , A^predictive_N)
        ...
        this -> get_R = [this]() -> Eigen::Matrix<double, rank_abc*predictive_N, rank_abc*predictive_N>
        {
            ...
            Eigen::Matrix<double, rank_abc*predictive_N, rank_abc*predictive_N> result = (this->get_gamma()).transpose() * this->get_gamma() + lambda_u * (this->S).transpose() * (this->S);
            return result;
        };
