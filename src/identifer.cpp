...
//                         const double& we, const double& Ts)
    void Identifier::renew(const std::vector<double>& Iabc,const std::vector<int>&inputs, const double& Ts,
                 const double& we, const double& Vdc, const double&theta_ele , const double& u0)
    {
        ...
        double Ua = inputs[0] * Vdc / 2.0 - inputs[0]*inputs[0] * u0;
        double Ub = inputs[1] * Vdc / 2.0 - inputs[1]*inputs[1] * u0;
        double Uc = inputs[2] * Vdc / 2.0 - inputs[2]*inputs[2] * u0;
